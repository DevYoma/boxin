...
const DashboardTab = () => {
  ...
  useEffect(() => {
    ...
    const getStoreDetails = async () => {
      axios
        ...
        .then((res) => {
          // console.log("getStoreDetails", res.data);
  ...
  const mainTabsMapping = mainTabs.map((mainItem) => (
    <p key={mainItem.id}>
      <Link className="dashboardTab__link" to={`/app${mainItem.link}`}>
        {mainItem.name}
      </Link>
    </p>
  ));

  const eCommerceMapping = eCommerce.map((eCommerceItem) => (
    <p key={eCommerceItem.id}>
      <Link className="dashboardTab__link" to={`/app${eCommerceItem.link}`}>
        {eCommerceItem.name}
      </Link>
    </p>
  ));
    ...
    >
      ...
      <List style={{ background: "#12113d", color: "white", height: "100vh" }}>
        ...
        <ListItem>
          <a
            key="3"
            className="dashboardTab__link"
            href={`https://${storeDetails.store_domain}.boxin.site`}
            target="_blank"
          >
